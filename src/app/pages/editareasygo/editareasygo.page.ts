...
import { Profile, PutSolicitudCotizacion, Easygo } from '../../models/data.model';
...
@Component({
  selector: 'app-editareasygo',
  templateUrl: './editareasygo.page.html',
  styleUrls: ['./editareasygo.page.scss'],
})
export class EditareasygoPage implements OnInit {
  usuario: Profile = {
    id: '',
    descripcion: '',
    name: '',
  ...
  EasyGoData() {
    this.geteasygodata = this.userService.getDataEasyGo(this.easygoid).subscribe( easygo => {
      const data = easygo.payload.data() as Easygo;
      console.log(data);

      this.descripcion = data.descripcion;
      this.geoAddress = data.direccion;
      this.geoLatitude = data.latitud;
      this.geoLongitude = data.longitud;
      this.upload = data.imagenes;
    });
  }
  ...
  guardarSolicitud() {
    ...
    this.userService.editarEasyGo(this.easygoid, data)
    .then(res => {
      this.volverEasyGo();
    }, err => {
  ...
  async volverEasyGo() {
    const alert = await this.alertController.create({
      cssClass: 'my-custom-class',
      header: 'Listo',
      message: 'Tu promocion EasyGo se ha actualizado correctamente.',
      buttons: [
        {
          text: 'Ver EasyGo',
          role: 'cancel',
          cssClass: 'secondary',
          handler: (blah) => {
            this.navCtrl.navigateForward('/vereasygo/' + this.easygoid);
          }
        }
      ]
    });
